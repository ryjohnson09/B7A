################################################
# Name: RespRatio_B7A_Geo.R
# Author: Ryan Johnson
# Date Created: 8 February, 2019
# Purpose: Generate RR from geochip B7A data
################################################
...
geo_grouped <- geo_relabun %>%
  ...
  select(Gene_category, starts_with("B7A")) %>%
  # Make long
...
ggsave(plot = RR_plot, filename = "results/figures/RespRatio_Geo_geneCat_B7A.png", height = 8, width = 7)
